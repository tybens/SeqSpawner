...
def simulate_sequence_not_independently(length, pwm_motif, background_weights, dist_fn, verbose=False):
    ...
    for i in range(num_motifs):
        ...
        pos_motif = dist_fn()  # scipy.stats.rvs(...)

        if verbose: print(" position of motif {}: ".format(str(i + 1)) + str(pos_motif + len(sample)))
...
def simulate_sequence(length, pwm_motif, background_weights, resolve_overlap, dist_fn: object, total_tries=10,
                      verbose=False):
    ...
    if resolve_overlap == 'reject':
        ...
        sample = []  # initialize empty sequence list
        poss_motif = []
        pn = 0  # iterator for the pos_motifs list
        for i in range(num_motifs):
            pos_motif = dist_fn()  # scipy.stats.rvs(...)
