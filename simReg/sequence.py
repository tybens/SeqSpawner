...
def simulate_background_then_motif(pos_motif, motif_pwm, emissions, background_weights):
    ...
    # motif (the pos_motif modification in `._sample` takes into account the anchor and alignment of the motif object)
    for i in range(motif_pwm.shape[1]):
        seq.extend(numpy.random.choice(emissions, 1, p=motif_pwm[:, i]))
